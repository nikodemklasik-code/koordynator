...
function repositoryAttachment(repository: string, commit: string, context: string) {
  ...
  return { name: `GitHub-${safeRepo}-${commit.slice(0, 12)}.txt`, mimeType: "text/plain", size: data.length, dataUrl: `data:text/plain;base64,${data.toString("base64")}` };
}
...
export function createControlServer(options: ControlServerOptions): Server {
  ...
  const projects = new ProjectSourceService(stateDir);
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (method === "GET" && url.pathname === "/api/projects") return sendJson(response, 200, { projects: await projects.list() });
      ...
      const projectMatch = /^\/api\/projects\/([0-9a-f-]{36})$/i.exec(url.pathname);
      if ((method === "GET" || method === "HEAD") && projectMatch?.[1]) {
      ...
      if ((method === "GET" || method === "HEAD") && url.pathname === "/api/chat/models") return sendJson(response, 200, await modelCatalog.list());
      ...
      if (method === "POST" && url.pathname === "/api/chat/sessions") {
        ...
        const session = await chat.createSession(requestedModel ? safeChatModel(requestedModel) : undefined);
      ...
      if (method === "POST" && chatMessageMatch?.[1]) {
        ...
        const attachments = repoContext ? [...clientAttachments, repositoryAttachment(repoContext.repository, repoContext.commit, repoContext.context)] : clientAttachments;
        return sendJson(response, 202, await chat.startMessage(sessionId, payload.message, model, attachments, billing));
      }
      ...
      if ((method === "GET" || method === "HEAD") && url.pathname === "/api/integrations/github") return sendJson(response, 200, await github.status(url.searchParams.get("refresh") === "1"));
      ...
      const staticFiles: Record<string, { name: string; type: string }> = {
        ...
        "/chat-model-health.css": { name: "chat-model-health.css", type: "text/css; charset=utf-8" },
      ...
      if (error instanceof ChatServiceError || error instanceof GitHubConnectionError || error instanceof ChatModelCatalogError || error instanceof GitHubRepositoryContextError || error instanceof ProjectSourceError) {
