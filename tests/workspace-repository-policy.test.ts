...
describe("workspace repository policy", () => {
  it("binds Corporation and Harmonia Legal to different repositories", () => {
  ...
  it("stores short-lived one-shot grants without spilling into another repo or branch", async () => {
    ...
    await store.grant({ repository: "nikodemklasik-code/Harmonia-Legal-Platform", branch: "develop", source: "terminal", ttlMs: 60_000 });
    ...
    expect(grants.map((item) => [item.repository, item.branch])).toContainEqual(["nikodemklasik-code/Harmonia-Legal-Platform", "develop"]);
  });
});
